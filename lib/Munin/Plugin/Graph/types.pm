...
declare "HexStr", as Str, where { /^[0-9a-fA-F]{6}$/ if defined($_) };
...
declare "Positive", as Int, where { $_ > 0 };

declare "NonNegative", as Int, where { $_ >= 0 };
...
declare_coercion "StrFromList", to_type Str, from ArrayRef [Str], via { join " ", @${_} };

declare_coercion "FieldNameFromStr", to_type ValidFieldName, from Str, via {
    s/^[^A-Za-z_]/_/;
    s/[^A-Za-z0-9_]/_/g;
    return $_;
};

declare_coercion "HexStrFromStr", to_type HexStr, from Str, via {
    $_ = $1 if defined($_) and /^#([0-9a-fA-F]{6})$/;
    return $_;
};

declare_coercion "ValueFromUndef", to_type ValueType, from Undef, via { 'U' };
